...
수업 코드는 **강사가 수업을 생성할 때 자동으로 만들어지는 8자리 고유 코드**입니다.

```
예시: ABCD1234, XYZ78945, QWE23456
```
...
### 3. 고유성 보장
- Firestore 문서 ID로 사용
- 중복 시 자동으로 덮어쓰기 (확률 극히 낮음)
- 예상 조합 수: 약 **2.8조 개** (32^8)
...
🎓 수업 코드 입력
┌────────────────┐
│ ABCD1234       │ ← 강사가 알려준 수업 코드
└────────────────┘
┌────────────────┐
│ 0001           │ ← 학생 번호 (강사가 지정 또는 자율)
└────────────────┘
```
...
O0OO0000  ← 0과 O 혼동
I1II1111  ← I와 1 혼동
12345678  ← 숫자만 (실제로는 생성 가능하지만 비권장)
```
