...
**0. P0 패키지: "사람 그릴까 말까" 자동 판단 (2026-01-13)** 🚀🔥🆕
- **핵심 기능**:
  - ✅ 사람 포함 여부 자동 감지 (이름, 대명사, 직업 키워드)
  - ✅ 연령 추론 방지 ("할머니/할아버지" 명시되지 않으면 노년 금지)
  - ✅ 프롬프트 자동 생성 (허술한 입력도 안정적인 결과)
- **판단 규칙**:
  - 사람 없음 키워드 → 풍경/사물만
  - "아이/어린이" → 어린이 포함, 노년 금지
  - "할머니/할아버지" → 노년 포함 허용
  - 대명사만 ("나", "우리") → 연령 중립 (노년 금지)
- **적용 범위**: 글쓰기 이미지, 동화 이미지 모두 적용
- **상세 문서**: `P0_PACKAGE_TESTING.md`

**1. 연령 중립 이미지 생성 시스템 (2026-01-13)** 🔥🆕
...
- **📤 파일 업로드 제출** (2026-03-10 추가): 
  - 앱에서 작품을 만들지 않아도 직접 이미지 업로드하여 제출 가능
  - 외부에서 촬영/제작한 이미지 제출 지원
  - 10MB 이하 이미지 파일 (PNG, JPG, JPEG)
  - 미리보기 + 제목 입력 후 즉시 제출
...
3. **작품 제출 (2가지 방법)**:
   - **방법 1**: 앱에서 작품 만들기 → 내 작품 저장 → 설정에서 선택하여 제출
   - **방법 2 (신규)**: 설정 → 파일 업로드 제출 → 이미지 선택 → 제목 입력 → 제출 ✨
4. **내 작품 보기**: `artifactList`로 저장된 작품 목록 조회
5. **공유하기**: `artifactByShare`로 QR/링크를 통해 다른 기기에서 열람
6. **강사 확인**: 교사 갤러리 (`/gallery/{classCode}`)에서 모든 학생 작품 확인
7. **강사 다운로드**: `exportClassZip`으로 전체 학생 작품 ZIP 다운로드
...
- ✅ 강사용 ZIP 다운로드
- ✅ 자동 백업
- ✅ 파일 업로드 직접 제출 (2026-03-10) 🆕
  - 앱에서 만들지 않아도 외부 이미지 제출 가능
  - 종이에 그린 그림 사진 촬영 후 제출
  - 다른 앱에서 만든 작품 제출
  - 제출 실패 시 백업 방법 제공
...
### ✅ 파일 업로드 제출 기능 추가
- **위치**: 설정 → "📤 파일 업로드 제출하기" 섹션
- **기능**:
  - 이미지 파일 직접 선택 (PNG, JPG, JPEG)
  - 파일 크기 제한: 10MB
  - 미리보기 자동 생성
  - 제목 입력 (자동 기본값 제공)
  ...
  - 교사 갤러리 즉시 반영
- **사용 시나리오**:
  - 앱에서 AI 이미지 생성이 안 될 때
  - 종이에 그린 그림을 사진으로 촬영하여 제출
  - 다른 앱에서 만든 작품 제출
  - 기존 이미지를 빠르게 제출
- **URL**: https://567d5922.story-maker-4l6-01i.pages.dev
