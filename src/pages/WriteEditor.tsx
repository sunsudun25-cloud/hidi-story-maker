...
export default function WriteEditor() {
  ...
  const genreExamples: { [key: string]: string[] } = {
    ...
    drama: [
      "#1. 거실 - 낮\n\n할머니가 소파에 앉아 창밖을 바라본다.\n\n할머니: (독백) 비가 오네...",
      "손자가 현관문을 열고 들어온다.\n\n손자: 할머니, 다녀왔습니다!\n할머니: (반갑게) 어서 와. 밥은 먹었니?",
      "할머니: (웃으며) 오늘도 학교에서 재미있었어?\n손자: 네! 친구들이랑 축구했어요.\n\n할머니, 손자의 머리를 쓰다듬는다."
    ]
  };
...
2. **자연스러운 대화**
   - 노인이 실제로 사용하는 자연스러운 말투
   - 너무 격식적이지 않고 따뜻한 어조
   - 간결하고 명확한 대사

3. **행동 지시 (지문)**
   - 등장인물의 행동과 감정을 간결하게 묘사
   - 과도한 설명 금지
   - 시각적으로 표현 가능한 행동 중심
...
손자: (장난스럽게) 할머니, 뭐 하세요?
할머니: (웃으며) 네가 좋아하는 된장찌개 끓이는 중이야.

손자가 냄비 뚜껑을 열어본다.

**❌ 잘못된 예시:**
할머니가 음식을 조리하면서 복잡한 감정을 느끼고 있었고...
손자: 할머니, 제가 정말로 말씀드리고 싶었던 것은... (너무 격식적)

위 대본에 자연스럽게 이어질 장면(2~3개 대사)을 작성해주세요.
대본 형식을 정확히 지켜주세요.
` : `
다음은 사용자가 작성 중인 글입니다:
...
`;
      ...
      // 시(poem)와 드라마(drama) 장르는 문장 끝 검증 제외
      if (!isPoem && !isDrama) {
        // 🔥 강화된 문장 끝 검증 (시, 드라마가 아닐 때만)
        ...
        console.log(`📝 [이어쓰기] ${isPoem ? '시(poem)' : '드라마(drama)'} 장르 - 문장 끝 검증 생략`);
      }
...
**⚠️ 중요: 원문의 모든 내용을 빠짐없이 드라마 대본으로 변환하세요!**

1. **완전한 변환 (최우선)**
   - 원문의 **모든 문장**을 드라마 대본으로 표현
   - 2줄만 쓰고 끝내지 말고, 원문 전체를 변환할 것
   - 원문이 10줄이면 → 대본도 여러 장면으로 충분히 작성
   - 내용을 생략하지 말고 모두 포함

2. **형식 구조**
   - 장면 헤더: #번호. 장소 - 시간 (예: #1. 지영의 집 현관 - 아침)
   - 지문(행동): 괄호 없이 간결하게 (예: "지영이 우산을 집어든다.")
   - 대사: 인물명: (감정) 대사 내용
   - 독백: 인물명: (독백) 내면의 생각
   - 빈 줄로 각 요소 구분
...
3. **내용 변환 방법**
   ...
   - 감정/생각 → 독백으로 표현
   - 대화 가능한 부분 → 대사로 표현
   - 시간/장소 변화 → 새로운 장면으로 구분

4. **독백 활용**
   - 원문의 내면 묘사는 (독백)으로 처리
   - 예: "무겁다. 요즘 우산보다 훨씬 무겁다." 
     → 인물명: (독백) 무겁다... 요즘 우산보다 훨씬 무거워.
...
원문:
"현관 구석에 아버지 우산이 있다. 손잡이가 닳아서 검은 테이프가 둘려 있는 우산. 아버지는 돌아가셨는데 우산은 여전히 그 자리에 서 있다.

비가 오는 날이면 나는 새 우산을 두고 그것을 들고 나간다. 무겁다. 요즘 우산보다 훨씬 무겁다. 그래도 그 무게가 싫지 않다."

변환:
#1. 지영의 집 현관 - 아침

지영이 현관을 지나다 걸음을 멈춘다. 현관 구석에 낡은 우산이 서 있다.

지영: (독백) 아버지 우산이네...

지영이 우산을 집어든다. 손잡이에 검은 테이프가 감겨 있다.

지영: (조용히) 아버지는 돌아가셨는데... 우산은 여전히 여기 있네.

#2. 거리 - 비 오는 날 오후

비가 내린다. 지영이 현관에서 새 우산을 보고, 아버지의 낡은 우산을 든다.

지영: (우산을 들며) 무겁다...

지영, 우산을 펼치고 걸어간다.

지영: (독백) 요즘 우산보다 훨씬 무거워. 그래도 이 무게가 싫지 않아.

---

**다시 한 번 강조:**
- 원문의 **모든 내용**을 드라마 대본으로 변환하세요
- 2~3줄만 쓰고 끝내지 마세요
- 원문이 길면 여러 장면으로 나눠서 **전부** 변환하세요

위 원문을 드라마 대본으로 **완전히** 변환해주세요.
`;
...
`;
  ...
  const handleHandwritingInput = async () => {
    // 사용자에게 용도 선택
    const purpose = confirm("📸 사진 속 텍스트를 어디에 사용하시겠습니까?\n\n확인 = 이미지 생성 프롬프트로 사용\n취소 = 본문 내용으로 추가");
    const purposeLabel = purpose ? '이미지 프롬프트' : '본문 내용';
    
    // 손글씨/인쇄물 선택
    const typeChoice = confirm("📸 사진 속 텍스트 인식\n\n인쇄물(책, 신문 등)이면 '확인'을 누르세요.\n손글씨면 '취소'를 누르세요.");
    const type = typeChoice ? 'print' : 'handwriting';
    const typeLabel = typeChoice ? '인쇄물' : '손글씨';
    ...
    try {
      ...
      console.log("✅ [WriteEditor] 이미지 업로드 완료");

      // ✅ Vision API로 텍스트 분석
      console.log(`🔍 [WriteEditor] ${typeLabel} 텍스트 인식 시작... (목적: ${purposeLabel})`);
      const extractedText = await analyzeHandwriting(result.base64, type);
      
      console.log("✅ [WriteEditor] 텍스트 인식 완료:", extractedText.substring(0, 100));

      // 인식된 텍스트를 목적에 따라 배치
      if (extractedText && extractedText !== "텍스트를 찾을 수 없습니다") {
        if (purpose) {
          // 이미지 프롬프트로 사용
          setImagePromptMode("manual");  // 수동 입력 모드로 전환
          setCustomImagePrompt(extractedText);
          alert(`✅ 텍스트를 성공적으로 읽었습니다!\n\n인식된 내용:\n"${extractedText.substring(0, 100)}${extractedText.length > 100 ? '...' : ''}"\n\n이미지 생성 프롬프트에 입력되었습니다.\n아래 [🎨 AI 그림 만들기] 버튼을 눌러주세요.`);
        } else {
          // 본문 내용으로 추가
          setContent(content + (content ? "\n\n" : "") + extractedText);
          alert(`✅ 텍스트를 성공적으로 읽었습니다!\n\n인식된 내용:\n"${extractedText.substring(0, 100)}${extractedText.length > 100 ? '...' : ''}"\n\n내용이 편집기에 추가되었습니다. 필요하면 수정해주세요.`);
        }
      } else {
